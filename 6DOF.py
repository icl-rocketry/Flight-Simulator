...
sideA = 0.2854
topA = 0.0114
Ilr_dry = np.array([[6.55,0,0],
                    [0,6.55,0],
                    [0,0,0.0219]]) # moment of inertia of the rocket when dry
Ilr_fuel = np.array([[0.0329,0,0],
                     [0,0.0329,0],
                     [0,0,0.0029]]) # moment of inertia of the fuel
...
def dragCoef(mach, alpha): #TODO: with the new sim this can be calculated in a less dodgy way
    ...
    if mach < 0.04 :
        ...
        Cd1 = 0.524 + 0.115*mach**2 # valid for 0-0.8 mach, from openrocket sim TODO: make this more realistic at transonic speeds
    Cd = (1.37*np.sin(alpha)**4 + Cd1**2*np.cos(alpha)**4)**0.5 #when the rocket is at an angle, there is a side component of drag
...
def windy(t):
    ...
    return windSpeed * np.array([np.cos(windAngle), np.sin(windAngle), 0]) + gustSpeed * np.array([np.cos(gustAngle), np.sin(gustAngle), 0]) * (0.5 + 0.5*np.cos(2*np.pi*t*gustFreq))
...
def recalculate(state, t, dt, initialCall):
    ...
    Rt = (Rt_dry*m_dry + R_tank*(state[13]-m_dry))/state[13] # distance from center of mass to thrust vector at current time
    ...
    D_rotate = -np.cross(w, direction) * np.linalg.norm(w) * rho * Cd_cyl * radius/6 * ((length - Rt)**3 - Rt**3) # drag due to rotation - TODO: add fins
    ...
    I = Ilr_dry + m_dry*np.array([[(Rt-Rt_dry)**2,0,0],
                                  [0,(Rt-Rt_dry)**2,0],
                                  [0,0,0]]) + (m-m_dry)/(m_wet-m_dry)*(Ilr_fuel + (m-m_dry)*np.array([[(Rt-R_tank)**2,0,0],
                                                                                                      [0,(Rt-R_tank)**2,0],
                                                                                                      [0,0,0]])) #TODO: openrocket seems to disagree with this?
    #centre of pressure moves too - but this is more complicated
    Rpt = CoP(mach) # distance from thrust location to pressure vector
    Rp = Rt - Rpt
    M_forces = np.cross(T, Rt*direction) + np.cross(D+L, Rp*direction) # moment - thrust contribution is currently zero but this supports TVC
    M_rotation = -rho * Cd * w * np.linalg.norm(w) * radius/8 * length**4 # moment due to rotational drag for a cylinder.
    ...
    trackedValues = [diff_dr, pitch, roll, yaw, M, D, alpha, pitch_rate, roll_rate, yaw_rate, Rt, Rp, mach, T, L, I] # if we want to plot things not in the state vector, we can add them to this list
...
def RK4(state, dt, t): # other methods can be used but this is a good start
    ...
    # calculate the derivatives of the state vector
    k1, trackedValues = recalculate(state, t, dt, True) #dt as the 3rd argument is only used for calculating rates, not part of the RK4 method
...
def main():
    ...
    q_quat = quaternion.from_rotation_vector(np.array([alpha*np.cos(launchRailDirection), alpha*np.sin(launchRailDirection), 0]))
    ...
    axs[0,0].plot([t for t in state_dict], 
                    [state_dict[t][0] for t in state_dict], label='x')
    axs[0,0].plot([t for t in state_dict],
                    [state_dict[t][1] for t in state_dict], label='y')
    axs[0,0].plot([t for t in state_dict],
                    [state_dict[t][2] for t in state_dict], label='z')
    ...
    axs[0,1].plot([t for t in state_dict], 
                    [state_dict[t][3] for t in state_dict], label='x')
    axs[0,1].plot([t for t in state_dict],
                    [state_dict[t][4] for t in state_dict], label='y')
    axs[0,1].plot([t for t in state_dict],
                    [state_dict[t][5] for t in state_dict], label='z')
    axs[0,1].plot([t for t in state_dict],
                    [np.linalg.norm(state_dict[t][3:6]) for t in state_dict], label='total', color='black', alpha=0.2)
    ...
    axs[0,2].plot([t for t in state_dict], 
                    [tracked_dict[t][0][0] for t in state_dict], label='x')
    axs[0,2].plot([t for t in state_dict],
                    [tracked_dict[t][0][1] for t in state_dict], label='y')
    axs[0,2].plot([t for t in state_dict],
                    [tracked_dict[t][0][2] for t in state_dict], label='z')
    axs[0,2].plot([t for t in state_dict],
                    [np.linalg.norm(tracked_dict[t][0]) for t in state_dict], label='total', color='black', alpha=0.2)
    ...
    axs[0,3].plot([t for t in tracked_dict],
                    [np.linalg.norm(tracked_dict[t][13]) for t in tracked_dict], label='thrust')
    axs[0,3].plot([t for t in tracked_dict],
                    [np.linalg.norm(tracked_dict[t][5]) for t in tracked_dict], label='drag')
    axs[0,3].plot([t for t in tracked_dict],
                    [np.linalg.norm(tracked_dict[t][14]) for t in tracked_dict], label='lift')
    ...
    axs[1,0].plot([t for t in tracked_dict], 
                    [tracked_dict[t][1] for t in tracked_dict], label='pitch')
    axs[1,0].plot([t for t in tracked_dict],
                    [tracked_dict[t][2] for t in tracked_dict], label='roll')
    axs[1,0].plot([t for t in tracked_dict],
                    [tracked_dict[t][3] for t in tracked_dict], label='yaw')
    axs[1,0].plot([t for t in tracked_dict],
                    [tracked_dict[t][6] for t in tracked_dict], label='angle of attack', color='black', alpha=0.2)
    ...
    axs[1,1].plot([t for t in tracked_dict], 
                    [tracked_dict[t][7] for t in tracked_dict], label='pitch rate')
    axs[1,1].plot([t for t in tracked_dict],
                    [tracked_dict[t][8] for t in tracked_dict], label='roll rate')
    axs[1,1].plot([t for t in tracked_dict],
                    [tracked_dict[t][9] for t in tracked_dict], label='yaw rate')
    ...
    axs[1,2].plot([t for t in tracked_dict],
                    [tracked_dict[t][10] for t in tracked_dict], label='CoP')
    axs[1,2].plot([t for t in tracked_dict],
                    [tracked_dict[t][11] for t in tracked_dict], label='CoM')
    axs[1,2].set_xlabel('time (s)')
    axs[1,2].set_ylabel('distance from base (m)')
    axs[1,2].legend()

    #plot inertia tensor Ixx then Izz on a twin y axis
    axs[1,3].plot([t for t in tracked_dict],
                    [tracked_dict[t][15][0][0] for t in tracked_dict], label='Ixx/Iyy')
    axs[1,3].set_xlabel('time (s)')
    axs[1,3].set_ylabel('Ixx (kg m^2)')
    axs[1,3].grid(visible=True)
    axs2 = axs[1,3].twinx()
    axs2.plot([t for t in tracked_dict],
                    [tracked_dict[t][15][2][2] for t in tracked_dict], label='Izz', color='orange')
